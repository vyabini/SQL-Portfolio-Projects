...
VALUES
  ...
  ('500', '2', '2', '2020-04-15', '9999-12-31')) as A(customer_id, region_id, node_id, start_date, end_date);
...
VALUES
  ...
  ('189', '2020-01-27', 'withdrawal', '861')) as A(customer_id, txn_date, txn_type, txn_amount);
...
Select count(distinct(node_id)) as unique_node_ids from customer_nodes

--2. What is the number of nodes per region?
 
Select distinct region_id,node_id as unique_node_ids from customer_nodes order by region_id,node_id
Select region_id,count(distinct node_id) as unique_node_ids from customer_nodes group by region_id

--3. How many customers are allocated to each region?

Select region_id,COUNT(distinct customer_id) as no_of_customers from customer_nodes group by region_id;
...
with cte as(
Select *,
LAG(start_date) over(partition by customer_id,node_id order by start_date) st,
dateadd(day,1,LAG(end_date) over(partition by customer_id,node_id order by end_date)) et1
from customer_nodes where DATEPART(year,end_date)!='9999'),
cte1 as(
Select customer_id,region_id,node_id,
case when start_date=et1 then st else start_date end as startdate,end_date
from cte),
cte2 as(
Select customer_id,region_id,node_id,startdate,MAX(end_date) as enddate from cte1 
group by customer_id,region_id,node_id,startdate),
cte3 as(
Select *,DATEDIFF(day,startdate,enddate) as no_of_days from cte2)


Select AVG(no_of_days) as avg_days_to_relocate from cte3;
...
cte3 as(
Select *,DATEDIFF(day,startdate,enddate) as no_of_days from cte2)
Select distinct region_id, PERCENTILE_CONT(0.5) within group (order by no_of_days asc) over(partition by region_id) as median,
PERCENTILE_CONT(0.8) within group (order by no_of_days asc) over(partition by region_id) as percentile_80,
PERCENTILE_CONT(0.95) within group (order by no_of_days asc) over(partition by region_id) as percentile_95
from cte3


/*
update customer_nodes
set end_date = dateadd(year, (2020 - year(end_date)), end_date*/

													--B. Customer Transactions
--1. What is the unique count and total amount for each transaction type?

Select txn_type,SUM(txn_amount) as tot_amount, COUNT(txn_type) as trans_count from customer_transactions group by txn_type;
...
Select AVG(amount) as avg_amount,AVG(trans_count) as avg_trans_count from cte;

--3. For each month - how many Data Bank customers make more than 1 deposit and either 1 purchase or 1 withdrawal in a single month?

with cte as(
Select datepart(month,txn_date) as date_month,customer_id,case when txn_type='purchase' then COUNT(1) end as purchase,
case when txn_type='deposit' then COUNT(1) end as deposit,
case when txn_type='withdrawal' then COUNT(1) end as withdrawal 
from customer_transactions 
group by datepart(month,txn_date),customer_id,txn_type),
cte1 as(
Select date_month,customer_id,sum(purchase) as purchase,sum(deposit) as deposit,sum(withdrawal) as withdrawal from cte
group by date_month,customer_id)

Select date_month,COUNT(customer_id) as no_of_customers from cte1 where deposit>1 and (purchase>0 or withdrawal>0)
group by date_month;
...
with cte as(
Select customer_id,datepart(month,txn_date) as month_code,datename(month,txn_date) as month_name,
...
cte1 as(
Select customer_id,month_code,month_name,deposit-purchase-withdrawal as Balance from cte),

cte2 as(

Select *,SUM(Balance) over (partition by customer_id order by month_code rows between unbounded preceding and current row) as monthly_balance 
from cte1),

cte3 as(
Select * from (
Select distinct datepart(month,txn_date) as mon_c,DATENAME(month,txn_date) as mon_n from customer_transactions) A
cross join
(Select distinct customer_id from customer_transactions)B
),

cte4 as(
Select A.customer_id,A.mon_c,A.mon_n,c.monthly_balance,
LAG(c.monthly_balance) over(partition by A.customer_id order by A.mon_c) prev_mon_bal
from (select * from cte2) c
right join (Select * from cte3) AS A
on A.mon_c=c.month_code and A.customer_id=c.customer_id)


Select customer_id,mon_c,mon_n,coalesce(monthly_balance,prev_mon_bal) as monthly_balance from cte4
order by customer_id,mon_c
...
cte5 as(
Select customer_id,mon_c,mon_n,coalesce(monthly_balance,prev_mon_bal) as closing_balance,
ROW_NUMBER() over (partition by customer_id order by mon_c desc) as rn
from cte4),

cte6 as(
Select customer_id,mon_c,mon_n,closing_balance from cte5 where rn=1),

cte7 as(
Select customer_id,month_code,month_name,deposit as opening_balance,
ROW_NUMBER() over (partition by customer_id order by month_code) as rn
from cte),

cte8 as(
Select customer_id,month_code,month_name,opening_balance from cte7 where rn=1)

Select cte6.customer_id, opening_balance,closing_balance 
from cte6 
inner join cte8 
on cte6.customer_id=cte8.customer_id
where closing_balance>=1.05*opening_balance

/*
Create table sample(customer_id int,region_id int,node_id int,start_date date,end_date date)
drop table sample
insert into sample values(1,3,4,'2020-01-02','2020-01-03');
insert into sample values(1,3,4,'2020-01-04','2020-01-14')
insert into sample values(1,3,2,'2020-01-15','2020-01-16')
insert into sample values(1,3,5,'2020-01-17','2020-01-28');
insert into sample values(1,3,3,'2020-01-29','2020-02-18')
insert into sample values(1,3,2,'2020-02-19','2020-03-16')
insert into sample values(1,3,2,'2020-03-17','2020-12-31')

Select * from sample;
Select * from customer_nodes where customer_id=2;
...
group by customer_id,region_id,node_id,startdate)
Select *,DATEDIFF(day,startdate,enddate) as no_of_days from cte2
order by customer_id,startdate

--Approach2:
WITH cte as (
Select *
, MIN(start_date) over(partition by node_id order by node_id) as START_DATE_NEW
, MAX(end_date) over(partition by node_id order by node_id) as END_DATE_NEW
, DENSE_RANK()over(partition by node_id order by start_date) as RNK
, LEAD(start_date)over(partition by node_id order by start_date) as LEADSTARTDATE
, LEAD(end_date)over(partition by node_id order by end_date) as LEADENDDATE
From sample )
, cte1 as (
select * 
, CASE WHEN START_DATE_NEW != start_date THEN start_date ELSE START_DATE_NEW END as FINALSTARTDATE
, CASE WHEN END_DATE_NEW != LEADENDDATE then LEADENDDATE ELSE  END_DATE_NEW END as FINALENDDATE
from cte
where RNK = 1 OR LEADSTARTDATE IS NOT NULL )
select customer_id,region_id,node_id,FINALSTARTDATE,FINALENDDATE
from cte1
order by start_date
*/
